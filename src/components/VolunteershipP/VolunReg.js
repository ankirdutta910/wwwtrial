...
function VolunReg() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      db.collection("VolunteerShip").orderBy("date", "desc"),
      (snapshot) => {
        let list = [];
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete?")) {
      try {
        setOpen(false);
        await deleteDoc(doc(db, "VolunteerShip", id));
            ...
            {photos &&
              photos.map((item, index) => (
                <tr>
                  ...
                  <td style={{ textAlign: "left" }}>{item.work}</td>
                  <td style={{ textAlign: "left" }}>{item.qualification}</td>
                  <td style={{ textAlign: "center" }}>{item.exp}</td>
                  <td style={{ textAlign: "center" }}>{item.home}</td>
                  ...
                  >
                    ...
                    <Button
                      style={{ marginTop: "2px" }}
                      size="small"
                      onClick={() => navigate(`/editVolunteer/${item.id}`)}
                    >
