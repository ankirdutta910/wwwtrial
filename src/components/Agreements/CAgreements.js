...
function CAgreements() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      db.collection("CoordinatorsAgreement").orderBy("date", "desc"),
      (snapshot) => {
        let list = [];
                ...
                <tr>
                  ...
                  >
                    <Button
                      style={{ marginTop: "2px" }}
                      size="small"
                      onClick={() => navigate(`/edit_c_Agreements/${item.id}`)}
                    >
